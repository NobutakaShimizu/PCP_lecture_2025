...
\begin{definition}{制約グラフの冪乗}{power-of-constraint-graph}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item 一方の端点が$\Gamma(u)$, もう一方の端点が$\Gamma(v)$に属する元のグラフ$G$の全ての辺$e=\{s,t\}\in E$に対し, $(\sigma(s), \sigma'(t))\in c_e$が成り立つ.
    \end{itemize}
  \end{itemize}
\end{definition}
...
\begin{figure}[h]
  ...
  \caption{制約グラフ$G$のべき乗$G^t$の図例. 頂点$u,v$にそれぞれ$\sigma,\sigma'\in\Sigma^{\ceil{t/2}}$が割り当てられているとき, それが適切ならば, それぞれ関数$\sigma\colon \Gamma(u)\to\Sigma$と$\sigma'\colon \Gamma(v)\to\Sigma$と同一視できる. \label{fig:gap-amplification}}
\end{figure}
...
べき乗の制約グラフ$G^t$における割り当て$a'\colon V \to \Sigma^{\ceil{t/2}}$に対し, 元の制約グラフに対する割り当て$a\colon V\to\Sigma$を, 以下のように「多数決」によって定める:
