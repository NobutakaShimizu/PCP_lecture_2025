...
\begin{proof}
  ...
  \paragraph*{不満足値の性質.}
  あるユニバーサルな定数$\gamma>0$が存在して$\UNSAT(\varphi)\ge \gamma\cdot\UNSAT(G)$を示す (実際には$\gamma\approx 0.99$とできる).
  $\varphi$の任意の割り当て$a'\colon X\to\binset$を固定する.
  各頂点$u\in V$に対し, $a'$が定める
  $\pi_u$に属する変数の部分割り当てを$\pi'_u\in\binset^b$とする.
  元の制約グラフ$G$の各頂点$u\in V$に対し, $a(u)\in\Sigma$を「$\pi'_u$の復号化」, すなわち
  \begin{itemize}
    \item ある$\sigma\in\Sigma$が存在して$\dist(\pi'_u,\Had_b(\sigma)) \le 0.01$ならば, $a(u)=\sigma$
