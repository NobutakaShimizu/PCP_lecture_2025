...
\begin{definition}{制約グラフの冪乗}{power-of-constraint-graph}
  ...
  \begin{itemize}
    ...
    \item アルファベット集合は$\Sigma^{d^{\ceil{t/2}}}$とする. ここで, 頂点$u$に$\boldsigma=(\sigma_1,\dots,\sigma_{d^{\ceil{t/2}}}) \in \Sigma^{d^{\ceil{t/2}}}$が割り当てられたとき, 次の意味を持つ: 元の$d$-正則グラフ$G$上で頂点$u$から距離$\ceil{t/2}$以内の頂点の集合を$\Gamma(u)$とし, その元を頂点番号の大小順で並べて$\Gamma(u)=\qty{ v_1,\dots,v_\ell}$ とする (ここで$G$の$d$-正則性より$\ell \le d^{\ceil{t/2}}$である). このとき, $\boldsigma$は$v_1,\dots,v_\ell$の各頂点に$\Sigma$の元を割り当てる関数$\boldsigma \colon \Gamma(u)\to\Sigma$とみなし, $w\in\Gamma(u)$に対し$\sigma=\boldsigma(w)$を, \emph{$u$の$w$に対する意見}と呼ぶ.
    なお, $\ell < d^{\ceil{t/2}}$の場合は相異なる二つの$\boldsigma,\boldsigma'\in\Sigma^{d^{\ceil{t/2}}}$が同一の関数としてみなされることもある.
    \item 辺$\bolde = \{u,v\} \in \boldE$ に対応する制約$\boldc_{\bolde}\in \calC^t$は次の二つを満たす割り当て$(\boldsigma,\boldsigma')\in \Sigma^{d^{\ceil{t/2}}} \times \Sigma^{d^{\ceil{t/2}}}$によって満たされる:
    \begin{itemize}
      \item 全ての頂点$a\in \Gamma(u)\cap \Gamma(v)$に対し, $\boldsigma(a)=\boldsigma'(a)$が成り立つ.
      \item 一方の端点が$\Gamma(u)$, もう一方の端点が$\Gamma(v)$に属する元のグラフ$G$の全ての辺$e=\{s,t\}\in E$に対し, $(\boldsigma(s), \boldsigma'(t))\in c_e$が成り立つ.
    \end{itemize}
  \end{itemize}
\end{definition}
...
\begin{figure}[h]
  ...
  \caption{制約グラフ$G$のべき乗$G^t$の図例. 頂点$u,v$にそれぞれ$\boldsigma,\boldsigma'\in\Sigma^{\ceil{t/2}}$が割り当てられているとき, それぞれ関数$\boldsigma\colon \Gamma(u)\to\Sigma$と$\boldsigma'\colon \Gamma(v)\to\Sigma$とみなす. なお, 元のグラフ$G$が自己ループを持つことから, 距離$\ceil{t/2}$以内の全ての頂点に対して意見が定義される. \label{fig:gap-amplification}}
\end{figure}

パラメータ$t$は制約グラフのサイズ$\size(G)$には依存しない定数であることを常に想定する.
...
また, $\UNSAT(G)=0$ならば, 全制約を満たす割り当て$a\colon V \to \Sigma$を用いて$G^t$の全ての制約を満たす割り当て$a'\colon V \to \Sigma^{\ceil{t/2}}$を, 各頂点$u\in V$に対し
\begin{align*}
  \boldsigma(u)\colon s \mapsto a(s)
\end{align*}
となるような$\boldsigma\in\Sigma^{\ceil{t/2}}$を$a'(u)$と定めることによって得られる.
すなわち$\UNSAT(G^t)=0$である.

最後に$\UNSAT(G^t)$の下界を証明する.
頂点$u\in V$に対し, $V$値をとる確率変数$RW(u)$を, グラフ$G$上で頂点$u$から開始する長さ$\ceil{t/2}$のランダムウォークの最終到達頂点とする (長さとは辿った辺の本数のことである).
べき乗の制約グラフ$G^t$における割り当て$a'\colon V \to \Sigma^{\ceil{t/2}}$に対し, 元の制約グラフに対する任意の割り当て$a\colon V\to\Sigma$を, 「重み付き多数決」によって定める.
頂点$u\in V$に対し, $\boldsigma = a(u)$とし, 以下で定まる$\sigma$を$a(u)=\sigma$とする:
\begin{align}
  \sigma = \argmax_{\tau \in \Sigma} \qty{ \Pr\qty[ \boldsigma\qty(\RW(u)) = \tau ] }. \label{eq:argmax-of-RW}
\end{align}
常に$\RW(u)\in \Gamma(u)$であるから$\boldsigma\qty( \RW(u) )$は必ず存在する.
また, タイが発生した場合はその中から任意に選ぶ.
直感的には, $\boldsigma(u)$の中で最も登場頻度の高い意見を$a(u)=\sigma$としている.
このようにして定めた割り当て$a\colon V \to \Sigma$は, 適当な定数$\beta=\beta(\lambda,d,\abs{\Sigma})>0$に対して
\begin{align}
  \UNSAT(a';G^t) \ge \beta\sqrt{t}\cdot \min\qty{ \UNSAT(a;G), \frac{1}{t} }. \label{eq:lower-bound-of-UNSAT}
\end{align}
を満たすことを後で示す. $\UNSAT(a';G^t)=\UNSAT(G^t)$を満たす$a'$に対して\cref{eq:lower-bound-of-UNSAT}を適用し, さらに$\UNSAT(a;G)\le \UNSAT(G)$を代入すると$\UNSAT(G^t)$の所望の下界が得られて証明は完了する.

\paragraph*{\cref{eq:lower-bound-of-UNSAT}の証明.}

簡単のため$t\in\Nat$は偶数とする.
割り当て$a'\colon V\to \Sigma^{t/2}$を固定し, $a$を\cref{eq:argmax-of-RW}によって定める.
...
なお, $\abs{F} > \floor*{\frac{\abs{E}}{t}}$である場合は適当に部分集合をとって$\abs{F} \le \floor*{\frac{\abs{E}}{t}}$とする.
このとき$\UNSAT(a;G) $
