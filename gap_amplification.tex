...
\end{align}
と定める.
$\UNSAT_{G'}(a')=\UNSAT(G')$を満たす割り当て$a'\colon V'\to\Sigma$を任意に取る.
...
\begin{align*}
  ...
  &\ge \frac{(1-\lambda_0)d_0}{8}\UNSAT_G(a)\cdot \abs{E}  & & \because \text{ケース1の仮定} \\
  &\ge \frac{(1-\lambda_0)d_0}{8(d_0+1)}\UNSAT_G(a)\cdot \abs{E'}. & & \because\text{\cref{eq:bound_of_E'}}
\end{align*}
特にこれから$\UNSAT_{G'}(a') = \frac{\abs{F'}}{\abs{E'}}\ge \frac{(1-\lambda_0)d_0}{8(d_0+1)}\cdot\UNSAT(G')$が成り立つ.
...
\begin{align*}
  ...
  &\ge \abs{E}\UNSAT_G(a) - \frac{\UNSAT_G(a)}{2}\abs{E} & & \because \text{ケース2の仮定}\\
  &= \frac{\UNSAT_G(a)}{2}\abs{E} \\
  &\ge \frac{\UNSAT_{G}(a)}{2(d_0+1)}\abs{E'} & & \because\text{\cref{eq:bound_of_E'}}
\end{align*}
となり, これから
\begin{align*}
  \UNSAT_{G'}(a') = \frac{\abs{F'}}{\abs{E'}} \ge \frac{1}{2(d_0+1)}\cdot \UNSAT_G(a)
\end{align*}
が成り立つ.

ケース1,2より, \cref{eq:c_of_degree_reduction}で定まる定数$c'>0$に対して$\UNSAT_{G'}(a') \ge c'\cdot \UNSAT_G(a) \ge c'\cdot \UNSAT(G)$が成り立つ.
\end{proof}
...
すなわち$\abs{E}\UNSAT_G(a) = \abs{E'}\UNSAT_{G'}(a)$が成り立つので
\begin{align*}
  \UNSAT(G') = \min_{a}\UNSAT_{G'}(a) = \frac{\abs{E}}{\abs{E'}}\min_a \UNSAT_G(a) = \frac{d}{d+d_0+1}\UNSAT(G)
\end{align*}
が成り立つ.
