...
\item そうでない場合は$\abs{F} = \floor*{\frac{\abs{E}}{\ell}}$となるように$F$を任意にとる.
\end{itemize}
このとき,
\begin{align*}
  \frac{\abs{F}}{\abs{E}} \le \min\qty{ \frac{\abs{F_0}}{\abs{E}}, \frac{1}{\ell} } = \min\qty{ \UNSAT(a;G), \frac{1}{\ell} }
\end{align*}
が成り立つ.
べき乗グラフの辺$\bolde \in \boldE$は$G$上の長さ$\ell$の路に対応する.
従って$\bolde = (v_0,\dots,v_\ell)$ (ただし全ての$i\in[\ell]$に対し$\qty{v_{i-1},v_{i}}\in E$)と表し, 元のグラフの辺と区別するため$e\in E$を$G$辺, $\bolde\in\boldE$を$G^\ell$辺と呼ぶことにする.

$G^\ell$辺$\bolde=(v_0,\dots,v_\ell)$の$i$番目の辺$e_i:=\qty{v_{i-1},v_i}$は
\begin{itemize}
  \item $e_i \in F$
  \item $\vec{\sigma}(v_{0})_{v_{i-1}} = \sigma(v_{i-1})$ かつ $\vec(\sigma)(v_\ell)_{v_i} = \sigma(v_i)$
\end{itemize}
を全て満たすとき, \emph{悪い}ということにする.
$G^\ell$辺$\bolde$の中の悪い辺の個数を$B$, $B_i\in\binset$を辺$e_i$が悪いならば$B_i=1$, そうでないならば$B_i=0$とする.
一様ランダムに$G^\ell$辺$\bolde$を選ぶとき, $B$と各$B_i$は確率変数となる.
