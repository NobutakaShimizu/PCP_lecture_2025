...
\begin{lemma}{}{alphabet-reduction-kCSP}
  ...
  アルゴリズム$A'$は制約グラフ$G=\ip{(V,E),\Sigma,C}$を入力として受け取り,
  $k$-CSP $\varphi=(X,\Sigma_1,\calI,\calC')$を出力する.
  ただし, $\varphi$は
  \begin{itemize}
...
\begin{proof}
  ...
  ($\abs{\Sigma}$が2ベキでない場合はダミーの値を追加して$\abs{\Sigma}=\binset^b$とし, 各制約はダミーの値を無視する).
  ...
  このようにして得られた各$C_e$に対して\cref{thm:two-input-circuit-PCP}を適用する.
\end{proof}
