...
\subsection{方針}
  ...
  \begin{lemma}{回路の検証}{circuit-verification}
    あるベクトル$x\in\F_2^n$に対し, $\dist(\pi_x,\Had(x))\le 0.01$を満たすことが保証されているオラクル$\pi_x$への定数回のオラクルアクセスを用いて, 入力として与えられた回路$C$に対し, 
    以下を満たす多項式時間乱択オラクルアルゴリズム$V^{\pi_x,\pi_y}(C)$が存在する:
    \begin{itemize}
      \item $C(x)=1$ならば, ある$\pi_y\in \F_2^{n^2}$が存在して, $V^{\pi_x,\pi_y}(C)$は確率$1$で受理する.
      \item $C(x)=0$ならば, 任意の$\pi_y\in \F_2^{n^2}$に対し, $V^{\pi_x,\pi_y}(C)$は確率$0.99$で拒否する.
    \end{itemize}
  \end{lemma}
  ...
  \begin{proof}
    ...
    \begin{algo}{}{QuadEQ-PCP}
      PCP検証者$V^{\pi_x,\pi_y}(M,b)$を以下で記述する:
      \begin{enumerate}
        \item 入力として$M\in\F_2^{m\times n^2}$と$b\in\F_2^m$および証明として$\pi_x\colon\F_2^n\to\F_2$と$\pi_y\colon\F_2^{n^2}\to\F_2$へのオラクルアクセスを受け取る.
    ...
    この$y$に対して, $\pi_y=\Had_{n^2}(y)$とする (補題の仮定より, $\pi_x=\Had_n(x)$である).
    このとき,
    \begin{itemize}
      \item ステップ2では, $\pi_x,\pi_y$はどちらも線形関数であるため, 拒否する確率は$0$である.
    ...
    \begin{itemize}
      \item $\dist(\pi_x,\Had_n) \ge 0.01$ または $\dist(\pi_y,\Had_{n^2})\ge 0.01$ ならば, $V^{\pi_x,\pi_y}((f_k)_{k\in[m]})$はステップ3において確率$0.99$で拒否する. 以降では$\dist(\pi_x,\Had_n) < 0.01$かつ$\dist(\pi_y,\Had_{n^2}) < 0.01$とし, $\pi_x,\pi_y$に最も近い線形関数をそれぞれ$\Had_n(x),\Had_{n^2}(y)$とする.
    ...
    \end{itemize}
    以上より, $V^{\pi_x,\pi_y}((f_k)_{k\in[m]})$が一度のステップ3-5のループで拒否する確率は少なくとも$0.97$である.
  \end{proof}
