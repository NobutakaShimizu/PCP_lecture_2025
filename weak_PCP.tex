...
\subsection{方針}
  ...
  ここで$y_{i,j}=x_ix_j$としてこれらを並べたベクトル$y\in\F_2^{n^2}$を考える.
  ...
  \begin{proof}[\cref{thm:weak-PCP-theorem}の証明]
    判定問題$\QuadEQ$に対し, 所望のPCP検証者を構成すればよい.
    $\QuadEQ$のインスタンス$(f_k)_{k\in[m]}$に対し, \cref{eq:M-matrix,eq:b-vector}で定義される行列$M\in\F^{m\times n^2}$とベクトル$b\in\F^m$を考える.
    このとき, $(f_k)$がYesインスタンスであることと, \ref{item:jouken1}と\ref{item:jouken2}を満たす$x\in\F^n,y\in\F^{n^2}$が存在することは同値である.
    以下では, これらの条件を検証するPCP検証者$V^{\pi_x,pi_y}((f_k)_{k\in[m]})$を記述する.

    \begin{algorithm}{$\QuadEQ$に対するPCP検証者}{QuadEQ-PCP}
      \begin{enumerate}
        \item 入力として$(f_k)_{k\in[m]}$および証明として$\pi_x\colon\F^n\to\F$と$\pi_y\colon\F^{n^2}\to\F$へのオラクルアクセスを受け取る.
        \item 入力に対し\cref{eq:M-matrix,eq:b-vector}で定義される行列$M\in\F^{m\times n^2}$とベクトル$b\in\F^m$を計算する.
        \item \cref{thm:linearity-test}のアルゴリズムを$\pi_x$と$\pi_y$のそれぞれに対して実行し, それぞれが線形関数に$0.01$-近いかどうかを検証する. もしいずれかの実行においてこのアルゴリズムが拒否したら拒否する.
        \item \cref{lem:tensor-product-verification}の検証者を, オラクルとして$\pi_x$と$\pi_y$を用いて実行する. この検証者が拒否したら拒否する.
        \item \cref{lem:linear-equation-solution-verification}の検証者を, 入力として$(M,b)$, オラクルとして$\pi_y$を用いて実行する. この検証者が拒否したら拒否する.
        \item ここまでのステップで拒否していなければ受理して終了する.
      \end{enumerate}
    \end{algorithm}

    この検証者$V^{\pi_x,\pi_y}$が所望のPCP検証者であることを確認する.
    ...
    入力$(f_k)_{k\in[m]}$が$\QuadEQ$のYesインスタンスであるとする.
    このとき, ある$x\in\F^n$が存在して, $y_{i,j}=x_i\cdot x_j$で定まる$y\in\F^{n^2}$に対して$My=b$を満たす.
    この$x,y$に対して, それぞれ$\pi_x=\Had_n(x)$と$\pi_y=\Had_{n^2}(y)$とする.
    このとき,
    \begin{itemize}
      \item ステップ3では, $\pi_x,\pi_y$はどちらも線形関数であるため, 拒否する確率は$0$である.
      \item ステップ4では, 実際に全ての$i,j\in [n]$に対して$y_{i,j}=x_i\cdot x_j$が成り立つため, 拒否する確率は$0$である.
      \item ステップ5では, 実際に$y$は$My=b$の解であるため, 拒否する確率は$0$である.
    \end{itemize}
    以上より, $V^{\pi_x,\pi_y}((f_k)_{k\in[m]})$は確率$1$で受理する.

    次に$(f_k)$がYesインスタンスであるとする.
    任意の$\pi_x\colon\F^n\to\F$と$\pi_y\colon\F^{n^2}\to\F$を考える.

    \begin{itemize}
      \item $\dist(\pi_x,\Had_n) \ge 0.01$ または $\dist(\pi_y,\Had_{n^2})\ge 0.01$ ならば, $V^{\pi_x,\pi_y}((f_k)_{k\in[m]})$はステップ3において確率$0.99$で拒絶する. 以降では$\dist(\pi_x,\Had_n) < 0.01$かつ$\dist(\pi_y,\Had_{n^2}) < 0.01$とし, $\pi_x,\pi_y$に最も近い線形関数をそれぞれ$\Had_n(x),\Had_{n^2}(y)$とする.
      \item ステップ4は, \cref{lem:tensor-product-verification}の仮定が満たされているため, 確率$0.99$で拒否する. 以降ではさらに$x\in\F^n,y\in\F^{n^2}$が, 全ての$i,j\in [n]$に対し$y_{i,j}=x_i\cdot x_j$を満たすことが保証されているとする.
      \item ステップ5は, $My\ne b$であるため, \cref{lem:linear-equation-solution-verification}の検証者が確率$0.99$で拒否する.
    \end{itemize}
    以上より, $V^{\pi_x,\pi_y}((f_k)_{k\in[m]})$が一度のステップ3-5のループで拒絶する確率は少なくとも$0.97$である.
  \end{proof}
